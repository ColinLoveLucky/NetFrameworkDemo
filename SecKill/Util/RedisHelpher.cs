...
namespace SecKill.Util
{
	public class RedisHelper
	{
		...
		public virtual void Enqueue(string key, object data, int cacheTime=-1)
		{
			if (data == null)
			{
				return;
		...
		public virtual T DeQueue<T>(string key)
		{
			var rValue = _db.ListRightPop(key);
			if (!rValue.HasValue)
			{
				return default(T);
			}
